    ...
    *Not intended to be used directly.*
    ...
    def __request(self, name, json=True, **kwargs):
        try:
            ...
            return True, response.json() if json else response.text, response
        except HTTPError as e:
            return False, e.response.json(), e.response
...
class CookClient:
    ...
    def __init__(self, **kwargs):
        kwargs['url'] = '%s/swagger-docs' % kwargs['url']
    ...
    def create_jobs(self, jobs):
        ...
        """
        okay, result, response = self.client.rawscheduler.post_rawscheduler(json=False,
                                                                            RawSchedulerRequest={"jobs": jobs})
        return okay, result, response
    ...
    def delete_jobs(self, uuids):
        """Delete jobs by UUID."""
        okay, result, response = self.client.rawscheduler.delete_rawscheduler(job=uuids, json=False)

        return (okay, 'true', response) if okay else (okay, result, response)
    ...
    def await_jobs(self, uuids, timeout=30, interval=1, status='completed'):
        """Wait for jobs to reach the given status, returning a list of jobs if successful."""

        def all_jobs_completed():
            self.logger.info('Polling for job uuids %s' % uuids)
            okay, result, _ = self.poll_jobs(uuids)
            return okay and not [j for j in result if j.get('status') != status]

        ok = await_until(all_jobs_completed, timeout, interval)

        if ok:
            return self.poll_jobs(uuids)
        else:
            return False, '{"error": "timeout waiting for jobs}', None
    ...
    def await_instances(self, uuids, timeout=30, interval=5):
        ...
        def completed_instances():
            okay, instances_completed = self.poll_instances(uuids)

            if okay and not [i for i in instances_completed if not i.get('end_time')]:
                return instances_completed
    ...
    def retry_jobs(self, uuids, retries=1):
        """Update the retry count for jobs by UUID."""
        return self.client.retry.put_retry(UpdateRetriesRequest={'jobs': uuids, 'retries': retries})
...
class Instance:
    """A class for high level interaction with Cook job instances.
...
class Job:
    """A class for high level interaction with Cook jobs.
...
class Group:
    """A class for high level interaction with Cook groups.
